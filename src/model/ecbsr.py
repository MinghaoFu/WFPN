...
def make_model(args):
    return ECBSR(args.m_ecbsr, args.c_ecbsr, args.idt_ecbsr, args.act_type, args.scale[0], args.colors)
...
class ECBSR(nn.Module):
    def __init__(self, module_nums, channel_nums, with_idt, act_type, scale, colors):
        ...
        # backbone += [common.BasicBlock(common.default_conv, self.colors, self.channel_nums, 3, act=nn.PReLU(self.channel_nums))]
        # for i in range(self.module_nums):
        #     backbone += [common.BasicBlock(common.default_conv, self.channel_nums, self.channel_nums, 3, act=nn.PReLU(self.channel_nums))]
        # backbone += [common.BasicBlock(common.default_conv, self.channel_nums, self.colors*self.scale*self.scale, 3, act=nn.PReLU(self.colors*self.scale*self.scale))]
...
if __name__ == "__main__":
    # test ecbsr---Ycb(colors=1)
    net = ECBSR(4, 8, True, 'prelu', 2, 1)
    t = torch.rand(1,1,5,5)
    t = net(t)
    print(t.shape)